...
@Component({
  selector: 'app-apoderado-titular',
  templateUrl: './apoderado-titular.component.html',
  styleUrls: ['./apoderado-titular.component.css']
})
export class ApoderadoTitularComponent implements OnInit {
  ...
  constructor(
    ...
    if (localStorage.getItem('nuevaMatricula')) {
      ...
      this.matriculaService.obtenerApoderado(matriculaId, "titular").subscribe(resp => {
        if (resp.estado == "ok" && resp.valor.rut) {
          let apoderado = resp.valor;
          localStorage.setItem('apoderadoTitular', JSON.stringify(resp.valor));
          this.formulario.patchValue({
  ...
  guardarDatosDeApoderado() {
    ...
    this.matriculaService.guardarApoderado(apoderado).subscribe(resp => {
      if (resp.estado === "ok") {
        ...
        this.cargando = false;
        this.router.navigate(['/fichadematricula/apoderados'])
      } else {
